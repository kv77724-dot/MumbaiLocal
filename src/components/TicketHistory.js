  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={{flexDirection: 'row',padding:5}}>
        ...
        <Text style={{fontSize:26,fontWeight: 'bold'}}>{item?.destination}</Text>
      </View>
      ...
      <View style={{flexDirection: 'row',padding:10,justifyContent:'space-between',alignSelf:'stretch'}}>
        {(item?.ticketform === 'Book with ID (Paperless)') &&<Text style={{paddingVertical:8}}>Adhar/PAN No: {user?.idcardnumber}</Text>}
        {(item?.ticketform === 'Book & Print (Paper)') &&<Text style={{paddingVertical:8}}>Ticket token: {item?.otp}</Text>}
        {(item?.ticketform === 'Book & Travel (Paperless)') &&<Text style={{paddingVertical:8}}></Text>}
        <Text style={{fontSize:22,fontWeight:'bold'}}>Fare: Rs. {item?.fare}.00/-</Text>
      </View>

      <View style={{flexDirection: 'row',paddingBottom:10,justifyContent:'center',alignSelf:'stretch'}}>
