...
export default function BookTicket({navigation}) {
  ...
  let onPressNext = () => {
    if (deptFrom === goTo) {
      Alert.alert('Alert!', 'Source and Destination can not be same', [
        {text: 'OK'},
      ]);
    } else if (deptFrom === null || goTo === null) {
      Alert.alert('Alert!', 'Please select both Source and Destination', [
        {text: 'OK'},
      ]);
    } else {
      setShow(true);
  ...
  let onPressGetFare = () => {
    ...
    if (classType === 'second') {
      if (distance <= 8) fare = 5;
      else if (distance > 8 && distance <= 28) fare = 10;
      else if (distance > 28 && distance <= 52) fare = 15;
      else if (distance > 52 && distance <= 74) fare = 20;
      else if (distance > 74 && distance <= 80) fare = 25;
      else fare = 30;
    } else {
      if (distance <= 8) fare = 50;
      else if (distance > 8 && distance <= 14) fare = 70;
      else if (distance > 14 && distance <= 23) fare = 105;
      else if (distance > 23 && distance <= 42) fare = 150;
      else if (distance > 42 && distance <= 60) fare = 170;
      else if (distance > 60 && distance <= 88) fare = 200;
      else fare = 240;
    }
    ...
    let totalFare =
      fare * parseInt(noOfAdult) + Math.round(fare / 2) * parseInt(noOfChild);
    ...
    <ScrollView>
      <View style={styles.container}>
        <View style={styles.card}>
          ...
          {show === false && (
            <View style={[styles.button, {marginBottom: 80, marginTop: 50}]}>
                ...
                <View style={{flex: 1, paddingLeft: 5}}>
                  ...
                  <DropDownPicker
                    items={[
                      ...
                      {
                        label: 'Other UPI/ Net Banking/ Debit Card',
                        value: 'other',
                      },
                    ]}
                    containerStyle={{height: 30}}
